...
def _load_from_app(session: S) -> None:
    ...
    Uses all .sql files stored at ./example/models/**
    """
    load_sql_from_folder(session, 'example/models')
...
def sync(args: List[str]) -> None:
    ...
    schema defined at `./example/models/**/*.sql`.
    """
...
def pending(_: List[str]) -> None:
    ...
    application schema defined at `./example/models/**/*.sql`, then saves
